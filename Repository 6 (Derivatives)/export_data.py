...
Called by the GitHub Actions workflow daily.  Falls back to realistic
simulated market data when Yahoo Finance is unreachable.
"""
...
# Add repo root to path so we can import from sibling packages
...
# ---------------------------------------------------------------------------
# Simulate realistic daily moves when live data is unavailable
# ---------------------------------------------------------------------------

def _add_history(docs_path: str, today_payload: dict) -> list:
    """
    Load existing history from data.json and append today's snapshot.
    ...
    # Keep last 30 days, deduplicate by date
    seen = set()
    deduped = []
    for h in reversed(history):
        if h["date"] not in seen:
            seen.add(h["date"])
            deduped.append(h)
    history = list(reversed(deduped))[-30:]
    return history
...
def run():
    ...
    # Add per-ticker spot prices to payload
    payload["spot_prices"] = spot_prices
    payload["limits"] = {
        "delta_limit": config.DELTA_LIMIT,
        "vega_limit": config.VEGA_LIMIT,
    }
